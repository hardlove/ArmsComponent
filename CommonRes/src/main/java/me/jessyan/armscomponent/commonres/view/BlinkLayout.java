...
public class BlinkLayout extends LinearLayout {
    ...
    public BlinkLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs,
                R.styleable.public_blink_layout,
                0, 0);

        try {
            shimmerAnimationDuration = a.getInteger(R.styleable.public_blink_layout_blink_animation_duration, 1500);
            shimmerColor = a.getColor(R.styleable.public_blink_layout_blink_color, ContextCompat.getColor(context, R.color.public_shimmer_color));
        } finally {
            a.recycle();
        }
    }
